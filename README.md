...
- 你还在为`开始菜单`残缺不全的应用目录而捶胸顿足吗？
- 你还在为`系统设置-应用`界面堪称💩的应用管理界面而烦恼不已吗？
...
`GetYourWinApp`是一个极其简易但是好用的 WinAppInventory 工具，旨在**深度扫描、提取并可视化 Windows 系统中安装的所有应用程序**（包括 UWP 应用、传统可执行程序、系统快捷方式导向应用、Shell 命令导向应用、URL web document 导向应用，等等各种 APP），并为用户提供**可视化的检视列表**，展示所有已安装程序的各方面信息。
...
1. 从此仓库下载：下载 `.\GetYourWinApp.ps1` 脚本到本地（或下载提供的可执行程序）。
2. 找到下载内容：以管理员权限运行 PowerShell，导向到脚本所在地。（或找到下载的可执行程序）
3. 设置执行策略：`Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser`。
4. 执行脚本：`.\GetYourWinApp.ps1`。（或运行可执行程序）
5. 等待执行完成：需要约 10-20 sec 的时间（可执行文件执行完成后，会启动窗口供用户确认）. 
